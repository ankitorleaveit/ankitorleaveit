...
D = {
    ...
    "achievements":  6,
    "badges": [
        ("brain",   "GALAXY BRAIN",        "answers accepted"),
        ("shark",   "PULL SHARK",          "pull requests merged"),
        ("bolt",    "QUICKDRAW",           "issue closed in 5 min"),
        ("pair",    "PAIR EXTRAORDINAIRE", "co-authored commits"),
        ("rocket",  "YOLO",                "merged without review"),
        ("sponsor", "PUBLIC SPONSOR",      "sponsoring open source"),
    ],
...
def build_scan(path):
  ...
  <mask id="rainMask"><rect x="453" y="51" width="530" height="394" fill="url(#rainfade)"/></mask>
    ...
    log = [
        ...
        ("[ ok ] ", "verify achievements ", str(D["achievements"])),
...
def badge_glyph(kind):
    """Marks drawn around (0,0), each with its own micro-motion."""
    g = [f'<g fill="none" stroke="{ACCENT2}" stroke-width="2" stroke-linecap="round" '
         f'stroke-linejoin="round">']
    if kind == "brain":
        for k, rot in enumerate((-28, 28, 90)):
            g.append(f'<ellipse cx="0" cy="0" rx="19" ry="7" transform="rotate({rot})" opacity="0.9">'
                     f'<animateTransform attributeName="transform" type="rotate" from="{rot}" '
                     f'to="{rot+360}" dur="{9+k*2}s" repeatCount="indefinite"/></ellipse>')
        g.append(f'<circle cx="0" cy="0" r="4" fill="{ACCENT2}" stroke="none">'
                 f'<animate attributeName="r" values="3.6;5.2;3.6" dur="2.6s" calcMode="spline" '
                 f'keySplines="{EASE};{EASE}" repeatCount="indefinite"/></circle>')
    elif kind == "shark":
        g.append('<g><animateTransform attributeName="transform" type="translate" '
                 'values="0 0;0 -2.5;0 0" dur="3s" calcMode="spline" '
                 f'keySplines="{EASE};{EASE}" repeatCount="indefinite"/>'
                 '<path d="M-14 5 L2 -18 L7 5 Z"/></g>')
        for k, oy in enumerate((11, 18)):
            g.append(f'<g opacity="{1 - k*0.45}"><animateTransform attributeName="transform" '
                     f'type="translate" values="-10 0;0 0" dur="{2.2 + k*0.6}s" '
                     f'repeatCount="indefinite"/>'
                     f'<path d="M-28 {oy} q5 -5.5 10 0 t10 0 t10 0 t10 0 t10 0"/></g>')
    elif kind == "bolt":
        g.append(f'<path d="M4 -18 L-11 2 L-1 2 L-4 18 L12 -3 L2 -3 Z" filter="url(#bloom)">'
                 f'<animate attributeName="opacity" values="1;0.35;1;0.7;1" '
                 f'keyTimes="0;0.06;0.12;0.18;1" dur="2.4s" repeatCount="indefinite"/></path>')
    elif kind == "pair":
        for sgn in (-1, 1):
            g.append(f'<circle cx="{sgn*7}" cy="0" r="11">'
                     f'<animate attributeName="cx" values="{sgn*10};{sgn*5};{sgn*10}" dur="3.4s" '
                     f'calcMode="spline" keySplines="{EASE};{EASE}" repeatCount="indefinite"/></circle>')
        g.append(f'<path d="M0 -17 L0 17" opacity="0.4" stroke-dasharray="3 4">'
                 f'<animate attributeName="stroke-dashoffset" values="0;-14" dur="1.6s" '
                 f'repeatCount="indefinite"/></path>')
    elif kind == "rocket":
        g.append('<g><animateTransform attributeName="transform" type="translate" '
                 'values="0 2;0 -3;0 2" dur="3.2s" calcMode="spline" '
                 f'keySplines="{EASE};{EASE}" repeatCount="indefinite"/>'
                 '<path d="M0 -18 c7 7 9 16 7 24 l-7 4 -7 -4 c-2 -8 0 -17 7 -24 Z"/>'
                 '<path d="M-7 5 l-6 7 3 2"/><path d="M7 5 l6 7 -3 2"/>'
                 f'<circle cx="0" cy="-5" r="3" fill="{ACCENT2}" stroke="none"/>'
                 f'<path d="M-3 12 L0 21 L3 12" stroke="{ACCENT}" filter="url(#bloom)">'
                 f'<animate attributeName="d" values="M-3 12 L0 21 L3 12;M-4 12 L0 28 L4 12;'
                 f'M-3 12 L0 18 L3 12;M-4 12 L0 25 L4 12;M-3 12 L0 21 L3 12" dur="0.9s" '
                 f'repeatCount="indefinite"/></path></g>')
    elif kind == "sponsor":
        heart = ("M0 14 C-15 3 -16 -11 -6 -13 C-2 -14 0 -10.5 0 -8.5 "
                 "C0 -10.5 2 -14 6 -13 C16 -11 15 3 0 14 Z")
        g.append(f'<g><animateTransform attributeName="transform" type="scale" '
                 f'values="1;1.16;1;1.08;1" keyTimes="0;0.12;0.26;0.38;1" dur="1.5s" '
                 f'calcMode="spline" keySplines="{OUT};{EASE};{OUT};{EASE}" repeatCount="indefinite"/>'
                 f'<path d="{heart}" fill="{ACCENT2}" fill-opacity="0.18" filter="url(#bloom)"/>'
                 f'<path d="{heart}"/></g>')
        g.append(f'<circle cx="0" cy="0" r="16" stroke="{ACCENT}" stroke-width="1.2" opacity="0">'
                 f'<animate attributeName="r" values="14;26" dur="1.5s" calcMode="spline" '
                 f'keySplines="{OUT}" repeatCount="indefinite"/>'
                 f'<animate attributeName="opacity" values="0.6;0" dur="1.5s" calcMode="spline" '
                 f'keySplines="{EASE}" repeatCount="indefinite"/></circle>')
    g.append('</g>')
    return "".join(g)


def build_badges(path):
    W, H, CYCLE = 1000, 224, 12.0
    random.seed(5)
    out = []
    A = out.append
    names = ", ".join(b[1].title() for b in D["badges"])
    A(f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" '
      f'role="img" aria-label="GitHub achievements unlocked: {names}">')
    A('<title>achievements --unlocked</title>')
    A('<defs>' + DEFS_COMMON
      + f'<radialGradient id="bglow" cx="50%" cy="50%" r="50%">'
        f'<stop offset="0%" stop-color="{ACCENT}" stop-opacity="0.26"/>'
        f'<stop offset="100%" stop-color="{ACCENT}" stop-opacity="0"/></radialGradient>'
        f'<linearGradient id="rail" x1="0" y1="0" x2="1" y2="0">'
        ...
        f'<clipPath id="shellClip"><rect width="{W}" height="{H}" rx="14"/></clipPath></defs>')
    shell(A, W, H)
    for j, (cx, cy, sx, sy) in enumerate(((16, 16, 1, 1), (W-16, 16, -1, 1),
                                          (16, H-16, 1, -1), (W-16, H-16, -1, -1))):
        A(f'<path d="M{cx} {cy+sy*12} L{cx} {cy} L{cx+sx*12} {cy}" fill="none" stroke="{ACCENT}" '
          f'stroke-width="1.6" opacity="1">'
          f'<animate attributeName="opacity" values="0;0.9;0.9;0" keyTimes="0;0.06;0.95;1" '
          f'calcMode="spline" keySplines="{OUT};0 0 1 1;{EASE}" dur="{CYCLE}s" '
          f'begin="{j*0.08:.2f}s" repeatCount="indefinite"/></path>')
    n = D["achievements"]
    A(f'<text x="34" y="38" font-family="{MONO}" font-size="12" letter-spacing="2" '
      f'fill="{ACCENT}">ACHIEVEMENTS_UNLOCKED</text>')
    A(f'<text x="966" y="38" text-anchor="end" font-family="{MONO}" font-size="11" '
      f'fill="{DIM}">{n} / {n} VERIFIED</text>')
    A(f'<line x1="34" y1="48" x2="966" y2="48" stroke="{ACCENT}" opacity="0.5" '
      f'stroke-dasharray="932">'
      f'<animate attributeName="stroke-dashoffset" values="932;0;0;932" keyTimes="0;0.07;0.95;1" '
      f'calcMode="spline" keySplines="{OUT};0 0 1 1;{EASE}" dur="{CYCLE}s" '
      f'repeatCount="indefinite"/></line>')

    badges = D["badges"]
    CYB, R = 110, 42
    XS = [(i + 0.5) * W / len(badges) for i in range(len(badges))]

    A(f'<line x1="{XS[0]:.0f}" y1="{CYB}" x2="{XS[-1]:.0f}" y2="{CYB}" stroke="url(#rail)" '
      f'stroke-width="1.2" stroke-dasharray="4 7" opacity="0.8">'
      f'<animate attributeName="stroke-dashoffset" values="0;-110" dur="5s" '
      f'repeatCount="indefinite"/></line>')
    A(f'<circle r="3" fill="{ACCENT2}" filter="url(#bloom)" opacity="0.9">'
      f'<animateMotion dur="{CYCLE/2}s" repeatCount="indefinite" calcMode="spline" '
      f'keyPoints="0;1" keyTimes="0;1" keySplines="{EASE}" '
      f'path="M{XS[0]:.0f} {CYB} L{XS[-1]:.0f} {CYB}"/>'
      f'<animate attributeName="opacity" values="0;0.95;0.95;0" keyTimes="0;0.06;0.9;1" '
      f'dur="{CYCLE/2}s" repeatCount="indefinite"/></circle>')

    for i, (kind, name, sub) in enumerate(badges):
        cx = XS[i]
        reveal = 0.05 + i * 0.05
        times = f"0;{reveal:.3f};{reveal+0.05:.3f};0.95;1"
        A(f'<g opacity="1"><animate attributeName="opacity" values="0;0;1;1;0" '
          f'keyTimes="{times}" calcMode="spline" keySplines="0 0 1 1;{OUT};0 0 1 1;{EASE}" '
          f'dur="{CYCLE}s" repeatCount="indefinite"/>')
        A(f'<g><animateTransform attributeName="transform" type="translate" '
          f'values="0 0;0 -6;0 0" dur="{5.4 + i*0.35:.2f}s" begin="-{i*0.6:.2f}s" '
          f'calcMode="spline" keySplines="{EASE};{EASE}" repeatCount="indefinite"/>')
        A(f'<circle cx="{cx:.0f}" cy="{CYB}" r="{R+22}" fill="url(#bglow)">'
          f'<animate attributeName="opacity" values="0.35;1;0.35" dur="{CYCLE/2}s" '
          f'begin="-{i*1.0:.2f}s" calcMode="spline" keySplines="{EASE};{EASE}" '
          f'repeatCount="indefinite"/></circle>')
        A(f'<polygon points="{hexagon(cx, CYB, R)}" fill="{INNER}" stroke="{BORDER}" '
          f'stroke-width="1.4"/>')
        per = 6 * R
        A(f'<polygon points="{hexagon(cx, CYB, R)}" fill="none" stroke="{ACCENT}" '
          f'stroke-width="2.2" stroke-dasharray="{per*0.3:.0f} {per*0.7:.0f}" stroke-linecap="round">'
          f'<animate attributeName="stroke-dashoffset" from="{per:.0f}" to="0" dur="{CYCLE/2}s" '
          f'begin="-{i*0.8:.2f}s" repeatCount="indefinite"/></polygon>')
        A(f'<g><animateTransform attributeName="transform" type="rotate" from="0 {cx:.0f} {CYB}" '
          f'to="-360 {cx:.0f} {CYB}" dur="{34 + i*3}s" repeatCount="indefinite"/>'
          f'<polygon points="{hexagon(cx, CYB, R-8, 12)}" fill="none" stroke="{DIM}" '
          f'stroke-width="1" opacity="0.75" stroke-dasharray="10 7"/></g>')
        A(f'<g transform="translate({cx:.0f} {CYB-2})">{badge_glyph(kind)}</g>')
        A(f'<polygon points="{hexagon(cx, CYB, R)}" fill="{ACCENT2}" opacity="0">'
          f'<animate attributeName="opacity" values="0;0.42;0;0" keyTimes="0;0.02;0.1;1" '
          f'dur="{CYCLE}s" begin="{i*0.5:.2f}s" calcMode="spline" '
          f'keySplines="{OUT};{EASE};0 0 1 1" repeatCount="indefinite"/></polygon>')
        A(f'<g transform="translate({cx:.0f} {CYB})">'
          f'<polygon points="{hexagon(0, 0, R)}" fill="none" stroke="{ACCENT2}" stroke-width="2" '
          f'opacity="0">'
          f'<animateTransform attributeName="transform" type="scale" values="1;1.4" dur="1.8s" '
          f'begin="{i*0.5:.2f}s" calcMode="spline" keySplines="{OUT}" repeatCount="indefinite"/>'
          f'<animate attributeName="opacity" values="0.75;0" dur="1.8s" begin="{i*0.5:.2f}s" '
          f'calcMode="spline" keySplines="{EASE}" repeatCount="indefinite"/></polygon></g>')
        for k in range(3):
            sx = cx + random.uniform(-30, 30)
            sy = CYB + random.uniform(10, 28)
            dur = 3.6 + k * 0.9
            A(f'<circle cx="{sx:.1f}" cy="{sy:.1f}" r="{random.uniform(1.1, 1.9):.1f}" '
              f'fill="{ACCENT2}">'
              f'<animate attributeName="cy" values="{sy:.1f};{sy-54:.1f}" dur="{dur:.1f}s" '
              f'begin="-{k*1.3 + i*0.4:.2f}s" calcMode="spline" keySplines="{EASE}" '
              f'repeatCount="indefinite"/>'
              f'<animate attributeName="opacity" values="0;0.85;0" dur="{dur:.1f}s" '
              f'begin="-{k*1.3 + i*0.4:.2f}s" repeatCount="indefinite"/></circle>')
        A('</g>')
        A(f'<text x="{cx:.0f}" y="{CYB+R+26}" text-anchor="middle" font-family="{MONO}" '
          f'font-size="10.5" letter-spacing="0.8" fill="{TEXT}">{esc(name)}</text>')
        A(f'<text x="{cx:.0f}" y="{CYB+R+41}" text-anchor="middle" font-family="{MONO}" '
          f'font-size="9" fill="{LABEL}">{esc(sub)}</text>')
        A('</g>')
    crt(A, W, H, "shellClip", CYCLE)
    A('</svg>')
    svg = "\n".join(out)
...
if __name__ == "__main__":
    ...
    print("badges.svg   ", build_badges(p("badges.svg")))
